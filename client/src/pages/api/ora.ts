    ...
    while (retries <= maxRetries) {
      try {
        if (USE_LOCAL_SERVER) {
          ...
          result = await callOraDirectly(message);
        }
...
async function callOraDirectly(userContent: string): Promise<AgentResponse> {
  const apiKey = process.env.Ora_API_KEY || "";
  const baseURL = process.env.Ora_BASE_URL || "https://api.ora.io/v1";

  // Initialize OpenAI client with Ora base URL
